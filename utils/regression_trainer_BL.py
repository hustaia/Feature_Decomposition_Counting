...
class RegTrainer_FD(Trainer):
    ...
    def train_eopch(self):
        ...
        for step, (inputs_high, inputs_low, points, targets, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                feat_high, feat_high_common, feat_high_unique, outputs_high, \
                    feat_low, feat_low_common, feat_low_unique, outputs_low = self.model(inputs_high, inputs_low)
                prob_list = self.post_prob(points, st_sizes)
                ...
                loss = density_loss + self.args.lamda * (consistency_loss + recon_loss)
                #print(density_loss.item(), consistency_loss.item(), recon_loss.item())
