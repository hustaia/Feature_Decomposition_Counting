...
__all__ = ['GramFormer', 'FD_GramFormer']
...
class VGG_Trans_FD(nn.Module):
    ...
    def forward(self, x_high, x_low=None):
        ...
        x_high, pe_high = self.encoder(x_high)   # transformer
        ...
        if x_low is not None:
            ...
            x_low, pe_low = self.encoder(x_low)   # transformer
            x_low = F.upsample_bilinear(x_low, size=(rh, rw))
            x_low = self.reg_layer_0(x_low)   # regression head
            return feat_high, feat_high_common, feat_high_unique, torch.abs(x_high), pe_high, \
                    feat_low, feat_low_common, feat_low_unique, torch.abs(x_low), pe_low
        
        return torch.abs(x_high), pe_high
...
def FD_GramFormer(topk, usenum):
    """VGG 19-layer model (configuration "E")
        model pre-trained on ImageNet
    """
    model = VGG_Trans_FD(make_layers(cfg['E']), topk, usenum)
    model.load_state_dict(model_zoo.load_url(model_urls['vgg19']), strict=False)
    return model