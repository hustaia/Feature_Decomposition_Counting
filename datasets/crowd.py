...
import random
import numpy as np
from datasets.low_illumination_degrading import Low_Illumination_Degrading
    ...
    def train_transform(self, img, keypoints):
        ...
        if len(keypoints) > 0:
            ...
            if random.random() > 0.5:
                img = F.hflip(img)
        img = transforms.ToTensor()(img)
        img_low = Low_Illumination_Degrading(img)
        return self.trans(img), self.trans(img_low), torch.from_numpy(keypoints.copy()).float(), \
