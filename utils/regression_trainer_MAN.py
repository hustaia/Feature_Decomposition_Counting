...
from models.vgg_c import MAN, FD_MAN
    ...
    def train_eopch(self):
        ...
        if True:
            ...
            for step, (inputs, inputs1, points, targets, st_sizes) in enumerate(self.dataloaders['train']):
                ...
                with torch.set_grad_enabled(True):
                    outputs, features = self.model(inputs)
                    outputs1, features1 = self.model(inputs1)
                    prob_list = self.post_prob(points, st_sizes)
...
class RegTrainer_FD(Trainer):
    ...
    def train_eopch(self):
        ...
        for step, (inputs_high, inputs_low, points, targets, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                feat_high, feat_high_common, feat_high_unique, outputs_high, features_high, \
                    feat_low, feat_low_common, feat_low_unique, outputs_low, features_low = self.model(inputs_high, inputs_low)
                prob_list = self.post_prob(points, st_sizes)
                density_loss = self.criterion(prob_list, targets, outputs_high) + self.criterion(prob_list, targets, outputs_low)
                #density_loss = self.criterion(prob_list, targets, outputs_low)
                loss_c = 0
                features_high.extend(features_low)
                ...
                loss = density_loss + loss_c + self.args.lamda * (consistency_loss + recon_loss)
